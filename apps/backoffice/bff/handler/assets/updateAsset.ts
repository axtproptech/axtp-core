...
const updateAssetBodySchema = object({
...
export const updateAsset: ApiHandler = async ({ req, res }) => {
  try {
    const { aliasId, ...d } = updateAssetBodySchema.validateSync(req.body);
    ...
    const transaction = (await assetService.updateAssetAlias({
      aliasId,
      data: {
